...
namespace SystemPack
{
	...
	CPatchSymbol* CPatchSymbol::CreateSymbol(const Common::CString& str)
	{
		...
		{
			Common::CString strNormalize = str.Copy( 1, str.Length() - 2 );
		...
		//
		Common::CArray<TSymBuilder*>& arrBuilders = TSymBuilder::arrBuilders;
		for (uint32 i = 0; i < arrBuilders.GetNum(); i++)
		{
			Common::CString& sSynonym = arrBuilders[i]->sSynonym;
			Common::CString  sType	  = sSynonym.GetWord(":", 1);
			Common::CString	 sSymbol  = sSynonym.GetWord(":", 2);
		...
		{
			Common::CString sLine = str.Copy( 1, str.Length()-2 );
			Common::CString sKeys	[5] = { "\\\\", "\\0", "\\n", "\\t", "\\\"" };
			char    sResults[5] = { '\\',	'\0',  '\n',  '\t',   '"'   };
			...
			Common::CString sCopied = sLine.Copy( 1, ulSize );
		...
		{
			Common::CString sPage	= str.GetWord( "x", 1 );
			Common::CString sOffset	= str.GetWord( "x", 2);
...
namespace SystemPack
{
	Common::CArray<TSymBuilder*> TSymBuilder::arrBuilders;

	TSymBuilder::TSymBuilder(const Common::CString& className, const Common::CString& synonym, TObjectCreator func)
	{
		for (uint32 i = 0; i < arrBuilders.GetNum(); i++)
		{
	...
	Common::CString TSymBuilder::GetClass()
	{
		return sClassName;
	}

	Common::CString TSymBuilder::GetSynonym()
	{
		return sSynonym;
	}

	TSymBuilder* TSymBuilder::GetByClassName(const Common::CString& className)
	{
		for (uint32 i = 0; i < arrBuilders.GetNum(); i++)
		{
			if( arrBuilders[i]->sClassName == className )
	...
	TSymBuilder* TSymBuilder::GetBySynonym(const Common::CString& synonym)
	{
		for (uint32 i = 0; i < arrBuilders.GetNum(); i++)
		{
			Common::CString sSynonym = arrBuilders[i]->sSynonym.GetWord(":", 2);
			if( sSynonym == synonym )
				return arrBuilders[i];
		}
		return Null;
	...
	Common::CString TSymBuilder::GetSymbolType(const Common::CString& synonym)
	{
		for (uint32 i = 0; i < arrBuilders.GetNum(); i++)
		{
			Common::CString sSynonym = arrBuilders[i]->sSynonym.GetWord(":", 2);
