...
namespace SystemPack
{
	...
	CPatchType* CPatchHex::operator + (CPatchType* symbol)
	{
		// Hex
		{
			...
			if( symHex )
			{
				...
				resultHex->Init();
				Common::CString		sLeft  ( (char*)		GetData(),		   GetSize() );
				Common::CString		sRight ( (char*)symHex->GetData(), symHex->GetSize() );
	...
	void CPatchHex::WriteOption()
	{
		...
		WriteOptionString( Common::CString( (char*)GetValue(), ulSize ), sOptionName.GetWord(":", -3), sOptionName.GetWord(":", -2), sOptionName.GetWord(":", -1) );
	}
