  ...
  CPatchType* CPatchFuncMessageBox::Execute() {
    ...
    for( uint32 i = 0; i < arrParameters.GetNum(); i++ ) {
      ...
      if( pHex ) {
        if( pHex->GetSize() == 0 )
          pHex->AutoSize();
        sWord += Common::CString( (text)pHex->GetValue(), pHex->GetSize() );
...
namespace SystemPack {
  ...
  CPatchType* CPatchFuncLoadLibrary::Execute() {
    ...
    Common::CString libName( (char*)pHex->GetValue(), pHex->GetSize() );
...
namespace SystemPack {
  ...
  static Common::CString SearchFile( const Common::CString& start, const Common::CString& file ) {
    // Files
    {
    ...
    // Dirs
    {
      ...
      while( explorer.FindNext() ) {
        if( explorer.IsFolder() ) {
          Common::CString sFile = SearchFile( start + explorer.FileName() + "\\", file );
          if( !sFile.IsEmpty() )
            return sFile;
        }
      }
  ...
  CPatchType* CPatchFuncModuleBase::Execute() {
    ...
    if( pHex->GetData() == Null ) {
      ...
      name = Common::CString( (char*)pHex->GetValue(), pHex->GetSize() );
      ...
      if( pBool ) {
        ...
        if( info.lpBaseOfDll == 0 ) {
          ...
          if( !module ) {
            ...
            if( !module ) {
              Common::CString sPath( (char*)pHex->GetValue(), pHex->GetSize() );
              if( sPath.Last() != '\\' )
                sPath += "\\";
...
namespace SystemPack {
  ...
  CPatchType* CPatchFuncHexViewBox::Execute() {
    ...
    Common::CString	sHexView;
    byte*	pBuff = (byte*)pHex->GetValue();
    for( uint32 i = 0; i < ulBytes; i++ ) {
      byte& mByte = pBuff[i];
      sHexView += Common::CString( mByte, 16 ) + " ";
    }
...
namespace SystemPack {
  ...
  CPatchType* CPatchFuncOptionDef::Execute() {
    ...
    Common::CString sOption = pType->GetOption();
...
namespace SystemPack {
  ...
  CPatchType* CPatchFuncImportSymbol::Execute() {
    ...
    HMODULE hModule = GetModuleHandle( Common::CString( (char*)pModule->GetValue(), pModule->GetSize() ) );

    CPatchHex* pHex = dynamic_cast<CPatchHex*>(arrParameters[1]);
    ASSERT_PATCH( pHex, "2-й Аргумент функции 'ImportSymbol' не является HEX" );
    Common::CString sSymbolName( (char*)pHex->GetValue(), pHex->GetSize() );
    ...
    if( ulNum == 3 ) {
      ...
      Common::CString sMangleType( (char*)pMangleType->GetValue(), pMangleType->GetSize() );
...
namespace SystemPack {
  ...
  CPatchType* CPatchFuncFindWindowHandle::Execute() {
    ...
    Common::CString sClassName(  (char*)pClassName->GetValue(),  pClassName->GetSize()  );
    Common::CString sWindowName( (char*)pWindowName->GetValue(), pWindowName->GetSize() );
