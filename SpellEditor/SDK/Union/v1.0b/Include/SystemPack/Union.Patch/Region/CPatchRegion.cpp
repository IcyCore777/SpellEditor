...
{
	Common::CString		CPatchRegion::sCurrentPatch = "Global";
	uint32		CPatchRegion::ulIterator = 0;
	uint32		CPatchRegion::ulEngineOffset = 0;
	Common::CDocument*	CPatchRegion::pCurrentRegionData = Null;

	Common::CString CPatchRegion::GetLastInfo()
	{
		if( pCurrentRegionData )
			return Common::CString::Combine("Region: %s\nLine: %i%r\"%s\"", 
				sCurrentPatch, 
	...
	CPatchRegion* CPatchRegion::DefineBlock (Common::CDocument& fileData, const uint32& hash)
	{
		...
		while( line )
		{
			...
			{
				Common::CString sHash = lineData.GetWord(sDefaultSpace, 2).GetPattern( sBrackets_Square[0], sBrackets_Square[1] ).Shrink();
				if( !sHash.IsEmpty() && sHash == Common::CString::Hex32(hash) )
				{
	...
	inline bool32 IsNotNumber (const Common::CString& line)
	{
		Common::CString sWord = line[0];
	...
	void CPatchRegion::ExecuteRegion ()
	{
		...
		{
			Common::CString& sLine = sRegionData[ulIterator];
			
			if( sLine.GetWord( sDefaultSpace ) == sPatchBlock[0])
			{
				Common::CString sPatchName = sLine.GetSymbolEx( Null, 0, &arrBrackets[1], 1);
	...
	uint32 CPatchRegion::ExecuteOnLine (uint32& iterator)
	{
		...
		Common::CArray<CPatchSymbol*> arrSymbols;
		...
		for (; iterator < sRegionData.GetNum(); iterator++)
		{
			...
			// бряки на строки
			//
			Common::CString& sLineData = sRegionData[iterator];
			...
			//
			Common::CString sKeyWord = sLineData.GetSymbolEx( arrResource, ulResourceNum, arrBrackets, 4, 1 );

			if( TSymBuilder::GetSymbolType( sKeyWord ) == "Type" )
			{
				// переменная делится на 2 типа - простая и ссылка
				Common::CString sKeyType = sLineData.GetSymbolEx( arrResource, ulResourceNum, arrBrackets, 4, 2 );

				if( sKeyType == sOperators[18] ) // @ ссылка
				{
					// имя переменной. должно быть НЕ число и начинаться НЕ с цифры
					Common::CString sKeyName = sLineData.GetSymbolEx( arrResource, ulResourceNum, arrBrackets, 4, 3 );
					...
					if( IsNotNumber(sKeyName) && !pAddress )
					{
						...
						//if( pInteger && !pAddress )
						{
							...
							if( pOperator )
							{
								Common::CString sINI = pOperator->GetOperator();
			...
			if( sKeyWord == "IF" )
			{
				...
				Common::CString sCond = sLineData.Copy( sKeyWord.Length(), sLineData.Length() - sKeyWord.Length() );
				...
				else
				{
					...
					{
						Common::CString& sPostWord = sRegionData[iterator].GetData().GetWord( sDefaultSpace );
						if( sPostWord == "ELSE" )
						{
							ExecuteOnLine( ++iterator );
							bConditions--;
