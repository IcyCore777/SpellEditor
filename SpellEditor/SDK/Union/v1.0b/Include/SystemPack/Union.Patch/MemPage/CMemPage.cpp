...
{
	Common::CArray<CMemPage*> CMemPage::arrPages;
	...
	CMemPage* CMemPage::Allocate (const uint32& index, const uint32& size)
	{
		...
		ASSERT( !pMemPage, "Страница памяти index[" + Common::CString(index, 16) + "] уже существует" );

		uint32 ulAddress = (uint32)VirtualAlloc( Null, size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE );
		ASSERT( ulAddress, "Невозможно получить страницу памяти index[" + Common::CString(index, 16) + "] size[" + Common::CString(size, 16) + "]" );
