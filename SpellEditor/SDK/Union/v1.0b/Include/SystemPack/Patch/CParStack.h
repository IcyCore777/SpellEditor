...
namespace SystemPack
{
	...
	{
		Common::CString sOperation;

	public:

		CParStackUncompiled ();
		CParStackUncompiled (const Common::CString& op);
		Common::CString GetSymbol ();
		void SetSymbol (const Common::CString& op);
		static CParStackUncompiled* CreateStack (const Common::CString& line); // создает некомпилированную очередь операторов
...
namespace SystemPack
{
	...
	public:
		...
		static CParStack* CreateStack (const Common::CString& line);
		virtual ~CParStack ();
	};
}