...
{
	Common::CString CPatch::sLastFile	= "";
	Common::CString CPatch::sLastRegion	= "";
	uint32  CPatch::ulLastLine	= None;

	void CPatch::Init (Common::CString fileName)
	{
		sFileName = fileName;
		...
		Common::CDocument sFile (sFileSrc);
		...
		for (uint32 i = 0; i < sFile.GetNum(); i++) 
		{
			...
			{
				Common::CString sHash = sFile[i].GetData().GetPattern("[", "]");
		...
		Common::CDocument sEngineRegion;
