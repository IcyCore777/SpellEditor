...
namespace SystemPack
{
	...
	CParStackUncompiled::CParStackUncompiled (const Common::CString& op) : CParStackBase ()
	{
		sOperation = op;
	}

	Common::CString CParStackUncompiled::GetSymbol ()
	{
		return sOperation;
	}

	void CParStackUncompiled::SetSymbol (const Common::CString& op)
	{
		sOperation = op;
	}
	...
	CParStackUncompiled* CParStackUncompiled::CreateStack (const Common::CString& line)
	{
		CParStackUncompiled* stack = new CParStackUncompiled;

		for (uint32 i = 1;; i++)
		{
			Common::CString sOp = line.GetSymbolEx( s_arrFilters, 4, s_arrBrackets, 5, i );
			if (sOp.IsEmpty())
				break;
...
#define OperatorID(id) op->sOperator == s_sOperators[id]
	...
	CParStack* CParStack::CreateStack (CParStackUncompiled* _stackUncompiled)
	{
		...
		while (stackUncompiled)
		{
			...
			if (!pHandler) 
			{
				Common::CString& symValue = stackUncompiled->GetSymbol();
				if (symValue.First() == '(' && symValue.Last() == ')')
				{
					Common::CString sSubStack = symValue.Copy( 1, symValue.Length() - 2 );
	...
	CParStack* CParStack::CreateStack (const Common::CString& line)
	{
