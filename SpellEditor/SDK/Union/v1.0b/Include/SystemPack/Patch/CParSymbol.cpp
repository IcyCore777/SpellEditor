...
namespace SystemPack
{
	...
	CParHandler* CParHandler::CreateHandler (const Common::CString& str)
	{
...
namespace SystemPack
{
	...
	Common::CArray<CParSymbol*> CParSymbol::arrSymbols;


	static const Common::CString sInt10Filter	= "1234567890-";
	static const Common::CString sInt16Filter	= "1234567890xabcdefXABCDEF";
	static const Common::CString sFloatFilter	= "1234567890.-";
	static const Common::CString sHexFilter		= "1234567890abcdefABCDEF ";
	...
	CParSymbol::CParSymbol (const uint32& type, const Common::CString& name)
	{
		if (!name.IsEmpty())
	...
	CParSymbol::CParSymbol (const Common::CString& type, const Common::CString& name)
	{
		if (!name.IsEmpty())
			for (uint32 i = 0; i < arrSymbols.GetNum(); i++)
				if (arrSymbols[i]->sName == name)
	...
	void CParSymbol::SetRoot (const uint32& type, const Common::CString& root)
	{
		ulRootType = type;
		sRoot	   = root;
	...
	void CParSymbol::SetOption (const Common::CString& str)
	{
		Common::CString sOptionData[3] = { NullStr, NullStr, NullStr };
		for (int32 i = 1; i <= 3; i++)
		...
		if (!sOptionData[1].IsEmpty() && !sOptionData[2].IsEmpty())
		{
			...
			{
				Common::CString val = Union.OptionReadString( sOptionData[1], sOptionData[2] );
	...
	static void32 HexToMem (const Common::CString& str, uint32& size)
	{ 
		Common::CArray<byte> arrBytes;
		for (uint32 i = 1;; i++)
		{
			Common::CString sByte = str.GetWord(" '", i);
	...
	static void32 StringToMem (const Common::CString& str, uint32& size)
	{
		size = str.Length()-2;
	...
	inline bool32 bIsFloat (const Common::CString& str)
	{
		return IsIncluded( sFloatFilter, str );
	}

	inline bool32 bIsHex (const Common::CString& str)
	{
		if (str.First() != '\'' || str.Last() != '\'')
			return False;
		return IsIncluded( sHexFilter, Common::CString(&str[1], str.Length()-2) );
	}

	inline bool32 bIsString (const Common::CString& str)
	{
		if (str.First() != '\"' || str.Last() != '\"')
			return False;
		return True;
	...
	ESymbolVarType CParSymbol::DefineSymbolType (const Common::CString& str)
	{
		if (str == "TRUE")
		{
			SetInt( TRUE );
	...
	void CParSymbol::ConvertToInt ()
	{
		...
		{
			Common::CString str( (char*)lpSymbol, 4 );
			SetInt( str.ToInt32() );
		}
		ApplyOption();
	}
	...
	void CParSymbol::ConvertToString ()
	{
		...
		if (ulSymType == VAR_TYPE_INT)
		{
			Common::CString sVal = *(int32*)lpSymbol;
		...
		{
			Common::CString sVal = *(real32*)lpSymbol;
	...
	void CParSymbol::ApplyOption ()
	{
		...
		if (!sOptionData[0].IsEmpty())
		{
			...
			if (opt)// && opt->IsExists( sOptionData[1], sOptionData[2] ))
			{
				...
				{
					opt->Write( Common::CString((char*)lpSymbol, ulSizeof), sOptionData[1], sOptionData[2] );
				}
				else
		...
		else if (Union.OptionIsExists( sOptionData[1], sOptionData[2] ))
		{
			if (ulSymType == VAR_TYPE_INT)
				...
				Union.OptionWriteString( sOptionData[1], sOptionData[2], Common::CString((char*)lpSymbol, ulSizeof) );
		}
	}
	...
	CParSymbol& CParSymbol::operator += (CParSymbol& sym)
	{
		...
		if (ulSymType == VAR_TYPE_HEX)
		{
			...
			if (sym.ulSymType == VAR_TYPE_INT)
			{
				...
				/*Common::CString sVal = *(int32*)sym.lpSymbol;
				lpSymbol = shi_realloc( lpSymbol, ulSizeof + sVal.Length() );
				memcpy( (void32)((int32)lpSymbol + ulSizeof), sVal.GetVector(), sVal.Length() );
				ulSizeof += sVal.Length();
			...
			if (sym.ulSymType == VAR_TYPE_FLOAT)
			{
				...
				/*Common::CString sVal = *(real32*)sym.lpSymbol;
				for (uint32 i = sVal.Length()-1; i != Invalid; i--)
				{
					char c = sVal[i];
					if (c == '.')
			...
			if (sym.ulSymType == VAR_TYPE_HEX)
			{
				...
				{
					value = Common::CString( (char*)sym.GetData(), sym.ulSizeof );
				}
				Common::CString source( (char*)lpSymbol, ulSizeof );
				source += value;
	...
	CParSymbol& CParSymbol::operator = (CParSymbol& sym)
	{
		...
		if (ulSymType == VAR_TYPE_HEX)
		{
			...
			{
				source = Common::CString( (char*)GetData(), ulSizeof );
			}
	...
	CParSymbol& CParSymbol::operator == (CParSymbol& sym)
	{
		...
		if (ulSymType == VAR_TYPE_HEX)
		{
			...
			if (sym.ulSymType == VAR_TYPE_HEX)
			{
				if (ulRootType == VAR_ROOT_EXTERNAL)
				{
					Common::CString str1( (char*)GetData(), sym.ulSizeof );
					Common::CString str2( (char*)sym.GetData(), sym.ulSizeof );
					newSym->SetInt( str1.CompareI( str2 ));
					return *newSym;
				}
				else
				{
					Common::CString str1( (char*)GetData(), ulSizeof );
					Common::CString str2( (char*)sym.GetData(), sym.ulSizeof );
					newSym->SetInt( str1.CompareI( str2 ));
					return *newSym;
				}
	...
	CParSymbol* CParSymbol::CreateSymbol (const Common::CString& str)
	{
	...
	Common::CString	CParSymbol::GetName	()
	{
		return sName;
	}
	...
	Common::CString CParSymbol::GetRoot ()
	{
		return sRoot;
	}
	...
	void CParSymbol::Print ()
	{
		...
		if (ulSymType == VAR_TYPE_HEX)
			cmd << (ulRootType == VAR_ROOT_EXTERNAL ? Common::CString(*(char**)lpSymbol, ulSizeof) : Common::CString((char*)lpSymbol, ulSizeof));
	}
...
namespace SystemPack
{
	const Common::CString s_sOperators [] = {
		"INT",		// 0
		"FLOAT",	// 1
		"HEX",		// 2
		"BOOL",		// 3
	...
	CParSymbol* CParOperator::Execute ()
	{
		...
		else if (sOperator == s_sOperators[29]) // MODULE_BASE
		{
			...
			HMODULE module = GetModuleHandle( Common::CString( (char*)nextSym->GetData(), nextSym->GetSize() ));
	...
	CParOperator* CParOperator::CreateOperator (const Common::CString& str)
	{
