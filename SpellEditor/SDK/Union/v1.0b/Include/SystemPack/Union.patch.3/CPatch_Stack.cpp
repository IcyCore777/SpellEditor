...
namespace SystemPack
{
	...
	CPatch_Stack* CPatch_Stack::CreateStack (const Common::CString& line)
	{
		...
		for (uint32 i = 1;; i++)
		{
			...
			CPatch_Symbol* pSymbol = CPatch_Operator::CreateSymbol( sWord );

			if( !pSymbol )
			{
				pSymbol = CPatch_Type::CreateSymbol( sWord );
			}
			if( !pSymbol )
			{
				 pSymbol = CPatch_Function::CreateSymbol( sWord );
				 if( pSymbol )
				 {
					Common::CString sParms = line.GetSymbolEx( arrResource, 4, arrBrackets, 4, ++i );
	...
	CPatch_Symbol* CPatch_Stack::GetData ()
	{
		return pObject;
	}

	void CPatch_Stack::SetData (CPatch_Symbol* object)
	{
		//object->AddRef();
		pObject = object;
	}

	CPatch_Type* CPatch_Stack::Execute ()
	{
		...
		while (stack)
		{
			...
			if( op )
			{
				...
				for (uint32 i = 0; i < arrSyms.GetNum(); i++)
				{
					if( arrSyms[i]->sClassName == op->GetName() )
					{
						CPatch_Symbol* sym = dynamic_cast<CPatch_Symbol*>( CClassDef::CreateClass( arrSyms[i]->pClassDef ));
	...
	CPatch_Type* CPatch_Stack::ExecuteLocal ()
	{
		/*CPatch_Symbol* sym = dynamic_cast<CPatch_Symbol*>( pObject );
		if( sym )
		{
