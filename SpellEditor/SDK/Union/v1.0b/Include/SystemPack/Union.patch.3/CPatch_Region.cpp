...
namespace SystemPack
{
	...
	uint32 CPatch_Region::ExecuteOnLine (uint32 line)
	{
		...
		for (uint32 i = line; i < sRegionData.GetNum(); i++)
		{
			...
			TPatch_SymDescriptor* pType = TPatch_SymDescriptor::Search( sKeyWord );
			...
			if( pType )
			{
				...
				if( sKeyType == sOperators[18] )
				{
					...
					if( !inlIsNumber(sKeyName) )
					{
						...
						CPatch_Type*   pSymbol = pStack->Execute();
						...
						CPatch_Type* pLink = CPatch_Type::CreateSymbol( sKeyWord );
				...
				else
				{
					...
					if( !inlIsNumber( sKeyName ))
					{
						CPatch_Type* pVar = CPatch_Type::CreateSymbol( sKeyWord );
