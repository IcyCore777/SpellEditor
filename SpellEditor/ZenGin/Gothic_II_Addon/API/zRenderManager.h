...
namespace Gothic_II_Addon {
  ...
  public:
	...
	void __fastcall spcBuildShader(zCMaterial*, zCTexture*, unsigned long, int, int, zCShader*&);
	void __fastcall spcSetAlphaFunc(zCShader *shader, zDWORD shaderStageIndex, zDWORD hardwareStageIndex);
	void __fastcall spcDrawVertexBuffer(zCVertexBuffer *vertexBuffer, int firstVert, int numVert, zWORD *indexList, zDWORD numIndex, zCShader *shader);
	void	__fastcall		spcDrawVertexBuffer(zCVertexBuffer *vertexBufferIn,
		int			firstVert,
		int			numVert,
		zWORD			*indexList,
		zDWORD			numIndex,
...
#define zERR_BOUNDS(var,l,h)			((void)(0))
  ...
  public:
	...
	void					AddStage(zCShaderStage *stage) { assert(numStages<MAX_STAGES);	 stageList[numStages++] = stage; };
